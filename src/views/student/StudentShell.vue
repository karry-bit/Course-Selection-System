    ...
    <header class="bg-white border-b border-slate-200 sticky top-0 z-40">
      <div class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
          ...
          <span class="text-lg font-bold text-slate-800">学生选课</span>
        </div>
...
const name = ref(localStorage.getItem('name') || '同学')

const nav = [
  { label: '浏览课程', to: '/student/courses' },
  { label: '课表', to: '/student/timetable' },
  { label: '成绩', to: '/student/scores' }
]
...
async function changePassword() {
  ...
  try {
    await api.studentChangePassword(pwdForm.oldPassword, pwdForm.newPassword)
    pwdMsg.value = '密码修改成功'
